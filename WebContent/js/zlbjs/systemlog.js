...
function search() {
	$("#myShowbody").html("");
	var starttime_systemlog = $("#starttime_systemlog").val();
	var endtime_systemlog = $("#endtime_systemlog").val();
	var emp_name = $("#emp_name").val();

	$.post("systemlog/searchsyslog.action", {
		"starttime_systemlog" : starttime_systemlog,
		"endtime_systemlog" : endtime_systemlog,
		"emp_name" : emp_name
	},
	...
	function(data) {
		...
		for (var i = 0; i < data.length; i++) {
			tr += "<tr>";
			tr += "<td>" + (i + 1) + "</td>";
			tr += "<td>" + data[i].log_time + "</td>";
			tr += "<td>" + data[i].emp_id + "</td>";

			tr += "<td>" + data[i].emptb.emp_name + "</td>";
			tr += "<td>" + data[i].log_even+ "</td>";
